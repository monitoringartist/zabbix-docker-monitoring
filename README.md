...
| Key | Description |
| --- | ----------- |
| **docker.discovery** | **LLD discovering:**<br>Only running containers are discovered.<br>Additional Docker permissions are needed, when you want to see container name (human name) in metrics/graphs instead of short container ID. |  
...
| **docker.inspect[fci,param1,\<param2\>]** | **Docker inspection:**<br>Requested value from Docker inspect structure is returned.<br>**param1** - name of 1st level JSON property<br>**param2** - optional name of 2nd level JSON property<br>For example: *docker.inspect[fci,NetworkSettings,IPAddress], docker.inspect[fci,State,StartedAt], docker.inspect[fci,Name]*<br>Note 1: Requested value must be plain text/numeric value. JSON objects/arrays are not supported.<br>Note 2: Additional Docker permissions are needed. |
| **docker.info[info]** | **Docker information:**<br>Requested value from Docker information structure is returned.<br>**info** - name of requested information, e.g. *Containers, Images, NCPU, ...*<br>Note: Additional Docker permissions are needed. |
| **docker.stats[fci,param1,\<param2\>,\<param3\>]** | **Docker container resource usage statistics:**<br>Docker version 1.5+ is required<br>Requested value from Docker statistic structure is returned.<br>**param1** - name of 1st level JSON property<br>**param2** - optional name of 2nd level JSON property<br>**param3** - optional name of 3rd level JSON property (not supported at the moment)<br>For example: *docker.inspect[fci,NetworkSettings,IPAddress], docker.inspect[fci,State,StartedAt], docker.inspect[fci,Name]*<br>Note 1: Requested value must be plain text/numeric value. JSON objects/arrays are not supported.<br>Note 2: Additional Docker permissions are needed.<br>Note 2: The most accurate way to get Docker container stats, but it's also the slowest (0.3-0.7s), because data are readed from on demand container stats stream. |
...
* docker.stats - implement 3rd parameter
