...
struct inspect_result zbx_module_docker_inspect_exec(AGENT_REQUEST *request) {
    ...
    if (strcmp(answer, "") == 0) {
        ...
        zabbix_log(LOG_LEVEL_DEBUG,
                   "docker.inspect is not available at the moment - some problem with Docker's socket API");
                ...
                if (request->nparam > 2) {
                    ...
                    if (SUCCEED != zbx_json_value_by_name(&jp_data2, param2, api_value2, buffer_size)) {
                        ...
                        if (SUCCEED != zbx_json_brackets_by_name(&jp_data2, param2, &jp_data_array)) {
                            free((void *) answer);
                            zabbix_log(LOG_LEVEL_WARNING, "Cannot find the [%s][%s] item in the received JSON object",
                                       param1, param2);
                            iresult.value = zbx_dsprintf(NULL,
                                                         "Cannot find the [%s][%s] item in the received JSON object",
                                                         param1, param2);
                            iresult.return_code = SYSINFO_RET_FAIL;
                            return iresult;
                        } else {
                            if (request->nparam < 4) {
                                ...
                                zabbix_log(LOG_LEVEL_DEBUG, "Item [%s][%s] found in the received JSON object: %s",
                                ...
                                while (NULL != (p_array = zbx_json_next(&jp_data_array, p_array))) {
                                    if ((result_array = strchr(p_array + 1, '"')) != NULL) {
                                        ...
                                        if (strncmp(value, get_rparam(request, 3), strlen(get_rparam(request, 3))) ==
                                            0) {
                                            // remove selector from returned value
                                            value += strlen(get_rparam(request, 3));

                                            zabbix_log(LOG_LEVEL_DEBUG,
                                                       "Item [%s][%s][%s] found in the received JSON object: %s",
                                                       param1, param2, get_rparam(request, 3), value);
                                        ...
                                        zabbix_log(LOG_LEVEL_WARNING,
                                                   "Cannot find the [%s][%s][%s] item in the received JSON object (non standard JSON array)",
                                                   param1, param2, get_rparam(request, 3));
                                        iresult.value = zbx_dsprintf(NULL,
                                                                     "Cannot find the [%s][%s][%s] item in the received JSON object (non standard JSON array)",
                                                                     param1, param2, get_rparam(request, 3));
                                        iresult.return_code = SYSINFO_RET_FAIL;
                                ...
                                free((void *) answer);
                                zabbix_log(LOG_LEVEL_WARNING,
                                           "Cannot find the [%s][%s][%s] item in the received JSON object (selector - param3 doesn't match any value)",
                                           param1, param2, get_rparam(request, 3));
                                iresult.value = zbx_dsprintf(NULL,
                                                             "Cannot find the [%s][%s][%s] item in the received JSON object (selector - param3 doesn't match any value)",
                                                             param1, param2, get_rparam(request, 3));
                        ...
                        if (request->nparam > 3) {
                            ...
                            if (SUCCEED != zbx_json_value_by_name(&jp_data3, param3, api_value3, buffer_size)) {
                                free((void *) answer);
                                zabbix_log(LOG_LEVEL_WARNING,
                                           "Cannot find the [%s][%s][%s] item in the received JSON object", param1,
                                           param2, param3);
                                iresult.value = zbx_dsprintf(NULL,
                                                             "Cannot find the [%s][%s][%s] item in the received JSON object",
                                                             param1, param2, param3);
                                iresult.return_code = SYSINFO_RET_FAIL;
                                return iresult;
                            } else {
                                free((void *) answer);
                                zabbix_log(LOG_LEVEL_DEBUG, "Item [%s][%s][%s] found in the received JSON object: %s",
                                           param1, param2, param3, api_value3);
                            ...
                            free((void *) answer);
                            zabbix_log(LOG_LEVEL_DEBUG, "Item [%s][%s] found in the received JSON object: %s", param1,
                                       param2, api_value2);
                    ...
                    free((void *) answer);
                    zabbix_log(LOG_LEVEL_WARNING,
                               "Item [%s] found in the received JSON object, but it's not plain value object", param1);
                    iresult.value = zbx_dsprintf(NULL,
                                                 "Can find the [%s] item in the received JSON object, but it's not plain value object",
                                                 param1);
                    iresult.return_code = SYSINFO_RET_FAIL;
                    return iresult;
                }
...
char *zbx_module_docker_get_fci(char *fci) {
    ...
    if (iresult.return_code == SYSINFO_RET_OK) {
        ...
        zabbix_log(LOG_LEVEL_DEBUG, "Default fci will be used, because zbx_module_docker_inspect_exec FAIL: %s",
                   iresult.value);
        return zbx_strdup(NULL, fci);
    }
}
...
int zbx_module_docker_up(AGENT_REQUEST *request, AGENT_RESULT *result) {
    ...
    if (cpu_cgroup == NULL) {
        if (zbx_docker_dir_detect() == SYSINFO_RET_FAIL) {
            ...
            SET_MSG_RESULT(result, zbx_strdup(NULL,
                                              "docker.up check is not available at the moment - no cpu_cgroup directory"));
            return SYSINFO_RET_FAIL;
        }
    }
    ...
    size_t filename_size =
            strlen(cgroup) + strlen(container) + strlen(stat_dir) + strlen(driver) + strlen(stat_file) + 2;
    if (c_prefix != NULL) {
...
int zbx_module_docker_dev(AGENT_REQUEST *request, AGENT_RESULT *result) {
    ...
    if (stat_dir == NULL || driver == NULL) {
        ...
        SET_MSG_RESULT(result,
                       zbx_strdup(NULL, "docker.dev metrics are not available at the moment - no stat directory"));
        return SYSINFO_RET_FAIL;
    }
    ...
    if (NULL == (file = fopen(filename, "r"))) {
        ...
        free(filename);
        SET_MSG_RESULT(result,
                       strdup("Cannot open Docker container stat file, maybe CONFIG_DEBUG_BLK_CGROUP is not enabled"));
        return SYSINFO_RET_FAIL;
    }
    ...
    while (NULL != fgets(line, sizeof(line), file)) {
        ...
        zabbix_log(LOG_LEVEL_DEBUG, "Container: %s; stat file: %s, metric: %s; value: %d", container, stat_file, metric,
                   value);
    ...
    if (SYSINFO_RET_FAIL == ret)
        SET_MSG_RESULT(result,
                       zbx_strdup(NULL, "Cannot find a line with requested metric in Docker container blkio file"));
...
int zbx_module_docker_mem(AGENT_REQUEST *request, AGENT_RESULT *result) {
    ...
    if (stat_dir == NULL || driver == NULL) {
        ...
        SET_MSG_RESULT(result,
                       zbx_strdup(NULL, "docker.mem metrics are not available at the moment - no stat directory"));
        return SYSINFO_RET_FAIL;
    }
    ...
    if (SYSINFO_RET_FAIL == ret)
        SET_MSG_RESULT(result, zbx_strdup(NULL,
                                          "Cannot find a line with requested metric in Docker container memory.stat file"));
...
int zbx_module_docker_cpu(AGENT_REQUEST *request, AGENT_RESULT *result) {
    ...
    if (stat_dir == NULL || driver == NULL) {
        ...
        SET_MSG_RESULT(result,
                       zbx_strdup(NULL, "docker.cpu metrics are not available at the moment - no stat directory"));
        return SYSINFO_RET_FAIL;
    }
    ...
    if (cpu_cgroup == NULL) {
        if (zbx_docker_dir_detect() == SYSINFO_RET_FAIL) {
            ...
            SET_MSG_RESULT(result, zbx_strdup(NULL,
                                              "docker.cpu check is not available at the moment - no cpu_cgroup directory"));
            return SYSINFO_RET_FAIL;
        }
    }
    // TODO CPU throttling
    ...
    */
    ...
    if (SYSINFO_RET_FAIL == ret)
        SET_MSG_RESULT(result, zbx_strdup(NULL,
                                          "Cannot find a line with requested metric in Docker container cpuacct.stat file"));
...
int zbx_module_docker_discovery_basic(AGENT_REQUEST *request, AGENT_RESULT *result) {
    ...
    if (stat_dir == NULL && zbx_docker_dir_detect() == SYSINFO_RET_FAIL) {
        zabbix_log(LOG_LEVEL_DEBUG,
                   "docker.discovery is not available at the moment - no stat directory - empty discovery");
...
int zbx_module_docker_discovery_extended(AGENT_REQUEST *request, AGENT_RESULT *result) {
    ...
    if (strcmp(answer, "") == 0) {
        zabbix_log(LOG_LEVEL_DEBUG,
                   "docker.discovery is not available at the moment - some problem with Docker's socket API");
        zbx_json_init(&j, ZBX_JSON_STAT_BUF_LEN);
        zbx_json_addarray(&j, ZBX_PROTO_TAG_DATA);
        zbx_json_close(&j);
        SET_STR_RESULT(result, zbx_strdup(NULL, j.buffer));
        ...
        if (SUCCEED != zbx_json_brackets_by_name(&jp_row, "Names", &jp_data2)) {
            ...
            if (request->nparam > 1) {
                ...
                if (iresult.return_code == SYSINFO_RET_OK) {
                    ...
                    zabbix_log(LOG_LEVEL_DEBUG,
                               "Default HCONTAINERID is used, because zbx_module_docker_inspect_exec FAIL: %s",
                               iresult.value);
                    zbx_json_addstring(&j, "{#HCONTAINERID}", names, ZBX_JSON_TYPE_STRING);
                }
            } else {
...
int zbx_module_docker_info(AGENT_REQUEST *request, AGENT_RESULT *result) {
    ...
    if (strcmp(answer, "") == 0) {
        zabbix_log(LOG_LEVEL_DEBUG,
                   "docker.info is not available at the moment - some problem with Docker's socket API");
        SET_MSG_RESULT(result,
                       strdup("docker.info is not available at the moment - some problem with Docker's socket API"));
        return SYSINFO_RET_FAIL;
    }
...
int zbx_module_docker_stats(AGENT_REQUEST *request, AGENT_RESULT *result) {
    ...
    if (strcmp(answer, "") == 0) {
        zabbix_log(LOG_LEVEL_DEBUG,
                   "docker.stats is not available at the moment - some problem with Docker's socket API");
        SET_MSG_RESULT(result,
                       strdup("docker.stats is not available at the moment - some problem with Docker's socket API"));
        return SYSINFO_RET_FAIL;
    }
    ...
    if (request->nparam > 1) {
        ...
        if (SUCCEED != zbx_json_value_by_name(&jp_data, param1, api_value, buffer_size)) {
            ...
            if (SUCCEED != zbx_json_brackets_by_name(&jp_data, param1, &jp_data2)) {
                zabbix_log(LOG_LEVEL_WARNING, "Cannot find the [%s] item in the received JSON object", param1);
                SET_MSG_RESULT(result,
                               zbx_dsprintf(NULL, "Cannot find the [%s] item in the received JSON object", param1));
                free((void *) answer);
                return SYSINFO_RET_FAIL;
            } else {
                // 2nd level
                if (request->nparam > 2) {
                        ...
                        if (SUCCEED != zbx_json_brackets_by_name(&jp_data2, param2, &jp_data3)) {
                            ...
                            if (request->nparam > 3) {
                                ...
                                if (SUCCEED != zbx_json_value_by_name(&jp_data3, param3, api_value3, buffer_size)) {
                                    ...
                                    zabbix_log(LOG_LEVEL_DEBUG,
                                               "Item [%s][%s][%s] found in the received JSON object: %s", param1,
                                               param2, param3, api_value3);
                                    SET_STR_RESULT(result, zbx_strdup(NULL, api_value3));
                                    free((void *) answer);
                                    return SYSINFO_RET_OK;
                                }
                            } else {
                                zabbix_log(LOG_LEVEL_DEBUG, "Item [%s][%s] found the received JSON object: %s", param1,
                                           param2, api_value2);
                    ...
                    zabbix_log(LOG_LEVEL_WARNING,
                               "Item [%s] found in the received JSON object, but it's not plain value object", param1);
                    SET_MSG_RESULT(result, zbx_dsprintf(NULL,
                                                        "Item [%s] found in the received JSON object, but it's not plain value object",
                                                        param1));
                    free((void *) answer);
                    return SYSINFO_RET_FAIL;
                }
...
int zbx_module_docker_cstatus(AGENT_REQUEST *request, AGENT_RESULT *result) {
    ...
    if (strcmp(state, "Up") == 0) {
        ...
        if (strcmp(answer, "") == 0) {
            zabbix_log(LOG_LEVEL_DEBUG,
                       "docker.cstatus is not available at the moment - some problem with Docker's socket API");
            SET_MSG_RESULT(result,
                           strdup("docker.cstatus is not available at the moment - some problem with Docker's socket API"));
            return SYSINFO_RET_FAIL;
        }
                ...
                if (strcmp(state, "All") == 0) {
                    ...
                    if (strcmp(state, "Paused") == 0) {
                        ...
                        while (NULL != (p = zbx_json_next(&jp_data, p))) {
                            ...
                            if (SUCCEED != zbx_json_value_by_name(&jp_row, "Status", status, cid_length)) {
                                zabbix_log(LOG_LEVEL_WARNING,
                                           "Cannot find the \"Status\" array in the received JSON object");
                                continue;
                            }
